...
\begin{figure}[H]
    ...
    \includegraphics[width=0.74\textwidth]{channel_distribution.pdf}
...
Following \citeA{amihud2002illiquidity}, baseline illiquidity is quantified via the median Amihud illiquidity ratio computed over the preceding 24 hours, establishing the baseline price impact of standard market activity:
\begin{equation}
...
Beyond market metrics, \textit{is\_weekend} equals 1 when $T_0$ falls on a Saturday or Sunday and 0 otherwise, absorbing weekend-specific differences in market conditions and trading behavior.

Finally, the preliminary synthetic-control stage discussed in the Appendix produced two hold-out fit diagnostics per event: \textit{Hold-Out} $R^2$ and \textit{Root Mean Square Error}. These variables originally checked how accurately the synthetic control matched the target coin's pre-pump price path, and they are retained as classification features to test whether pre-event price patterns predict pump \textit{Success}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw, rounded corners, minimum width=2.85cm, minimum height=0.9cm, align=center, font=\small},
        ...
        % Integration node
        \node[box, fill=purple!20, below=1.25cm of align, minimum width=4cm] (integrate) {Integrated Event Matrix\\(2,917 events)};
...
The data compilation workflow unifies unstructured Telegram chat logs, high-frequency OHLCV arrays, and historical CoinMarketCap metadata by separating Telegram classification, OHLCV alignment, and metadata enrichment before merging them into the final event matrix.
...
\bottomrule
\end{tabular}
\datasetnote{M denotes million, B denotes billion.}
\end{table}
...
Together, these records supply the treatment variables, confounders, and price data required for the causal analysis presented in Chapter~\ref{ch:methodology}, with \textit{Market Capitalization} also serving as the stratification key in Section~\ref{sec:stratified-estimation}.

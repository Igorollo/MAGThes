...
Two key variables used later as pre-reveal covariates are \textit{Coin Age} and \textit{Market Capitalization}. \textit{Coin Age} was computed from exchange listing dates, while \textit{Market Capitalization} was taken from the historical CoinMarketCap snapshot. Figure~\ref{fig:coin-age-distribution} shows a bimodal distribution in \textit{Coin Age}, with many targets around 1,000 days old. Figure~\ref{fig:market-cap-volume-scatter} shows that targets cluster most heavily in the 10 million USD to 100 million USD market-cap range.
...
Telegram signal features quantify the scale and intensity of social coordination (Table~\ref{tab:telegram-variables}). The engineered feature set includes \textit{Hype Intensity}, \textit{Urgency Index}, \textit{Instruction Density}, and \textit{Hype Volume}. In the final causal specification described in Chapter~\ref{ch:methodology}, \textit{Hype Volume} is excluded after collinearity screening, so the retained treatment vector contains \textit{Hype Intensity}, \textit{Urgency Index}, and \textit{Instruction Density}.
...
\textit{Urgency Index} & The ratio of uppercase characters in the primary signal message, used here as a simple proxy for urgency-style message framing. Prior work shows that pump organizers often rely on urgency and FOMO cues in their messages \cite{lamorgia2023doge}. \\
...
Beyond market metrics, temporal scheduling was encoded via the literal dataset column \texttt{is\_weekend}. This binary feature equals 1 if $T_0$ falls on a Saturday or Sunday, and 0 otherwise. It is included to absorb weekend-specific differences in market conditions and trading behavior.

Finally, the preliminary synthetic-control stage, discussed in the appendix, produced two hold-out fit diagnostics per event: \textit{Hold-Out $R^2$} and \textit{Root Mean Square Error}. These variables were originally used as quality checks to see how accurately the synthetic control matched the target coin's price path before the pump. However, they are also kept in the integrated dataset as extra features for the classification models. This helps test if pre-event price patterns contain information about post-signal pump \textit{Success}.
...
The final analytical sample consists of 2,917 complete pump-event records (see Section~\ref{sec:sample-selection}). Table~\ref{tab:sample-distribution} shows the breakdown of this final sample across the four \textit{Market Capitalization} strata used in the stratified estimation:
...
\begin{tabular}{|l|r|r|}
\toprule
\multicolumn{1}{|c|}{\textbf{\textit{Market Capitalization Stratum}}} & \multicolumn{1}{c|}{\textbf{N}} & \multicolumn{1}{c|}{\textbf{\%}} \\
\midrule
...
\quad \textit{Market Capitalization Stratum} & Micro & Small & Mid & Large \\
...
The sample is dominated by small- and mid-cap assets (73.7\% combined), but the strongest concentration is still in the 10 million USD to 100 million USD range discussed in Section~\ref{sec:metadata-constraints} and shown in Figure~\ref{fig:market-cap-volume-scatter}. Figure~\ref{fig:outcome-distribution-mcap} further illustrates how the \textit{10-Minute Logarithmic Return}, the primary outcome variable defined in Section~\ref{sec:outcome-variable}, is distributed across these strata. The figure shows the central 95\% of observations. Micro- and small-cap assets display greater variance than the larger strata, which is consistent with the thin-market constraints discussed above.

\begin{figure}[H]
\centering
\caption{\textit{10-Minute Logarithmic Return} distribution by \textit{Market Capitalization Stratum} (central 95\% of observations).}

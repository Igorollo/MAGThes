...
% 5. Custom commands for source notes under figures/tables
\newcommand{\source}[1]{%
  \par\vspace{0.5\baselineskip}%
  {\singlespacing\noindent\footnotesize\textit{Source:} #1\par}%
  \vspace{0.5\baselineskip}%
}
\newcommand{\datasetsource}{\source{Own elaboration based on the thesis dataset.}}
...
\begin{center}
  \textbf{Abstract}
\end{center}
\noindent This thesis studies Telegram-coordinated cryptocurrency pump-and-dump events. It links message features, high-frequency exchange data, and asset metadata to estimate how signal style affects 10-minute post-reveal returns. Using Double Machine Learning and heterogeneity checks, it shows that urgency matters mainly for small-cap assets, while effects weaken or turn negative in larger markets. A separate XGBoost model tests whether \textit{Success} can be predicted before the reveal. The results show that pump signals work as short-term coordination tools whose impact depends on market depth.
...
\end{center}
\begin{center}
  causal AI, cryptocurrency manipulation, pump-and-dump, Telegram signals, Double Machine Learning, market capitalization
\end{center}

\begin{center}
  (\textbf{Słowa kluczowe:} przyczynowa AI, manipulacja kryptowalutami, pump-and-dump, sygnały Telegram, Double Machine Learning, kapitalizacja rynkowa)
\end{center}
